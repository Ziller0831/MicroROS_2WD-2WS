...
void remote_control()
{
  ...
  linear_x = map(xboxController.xboxNotif.trigRT, 0, 1023, 0, 2)

      if (turning_mode == 0)
  {
    if (xboxController.xboxNotif.btnLS == true && xboxController.xboxNotif.trigRT == 0)
    {
      vel_direction = false;
    }
    elif (xboxController.xboxNotif.btnRS == true && xboxController.xboxNotif.trigRT == 0)
    {
      vel_direction = true;
    }
    ...
    {
      joy_command.linear_x = xboxController.xboxNotif.trigRT;
    }
    else
    {
      joy_command.linear_x = -xboxController.xboxNotif.trigRT;
