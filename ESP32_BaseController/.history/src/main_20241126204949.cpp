...
void initialize()
{
  ...
  if (R_Stepper && L_Stepper)
  {
    ...
    R_Stepper->setAcceleration(3000); // 設置加速度
    R_Stepper->setSpeedInHz(1200);    // 設置速度

    L_Stepper->setDirectionPin(DIR_L);
    L_Stepper->setAutoEnable(true);
    L_Stepper->setAcceleration(3000); // 設置加速度
    L_Stepper->setSpeedInHz(1200);    // 設置速度
  }
}
...
void moveBase()
{
  ...
  if (R_Stepper && L_Stepper)
  {
    ...
    // while (R_Stepper->isRunning() || L_Stepper->isRunning())
    // {
    //   // 等待運動完成
    // }
  }
