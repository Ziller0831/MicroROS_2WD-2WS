...
void loop()
{
  if(Serial.available() > 0){
    ...
    Seriall.println(RobotSteer.thetaR, RobotSteer.thetaL);
    Pulse[0] = RobotSteer.thetaR;
    Pulse[1] = RobotSteer.thetaL;
  }
...
void AckermannSteering(float Robot_yaw){
  ...
  RobotSteer.thetaR = tan(RobotSteer.L / (RobotSteer.R - (RobotSteer.T / 2)));
  RobotSteer.thetaL = tan(RobotSteer.L / (RobotSteer.R + (RobotSteer.T / 2)));
}