...
void loop()
{
  if(Serial.available() > 0){
    Robot_yaw = Serial.parseInt();
    RobotSteer.thetaR, RobotSteer.thetaL = AckermannSteering(Robot_yaw);
...
float AckermannSteering(float Robot_yaw){
  ...
  if(Robot_yaw != 0){
    ...
    thetaR = atan(RobotSteer.L / (RobotSteer.R - (RobotSteer.T / 2)));
    thetaL = atan(RobotSteer.L / (RobotSteer.R + (RobotSteer.T / 2)));
  }else{
    thetaR = 0;
    thetaL = 0;
  }
