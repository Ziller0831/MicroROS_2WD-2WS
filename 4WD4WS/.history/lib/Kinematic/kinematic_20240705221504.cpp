...
Kinematics::CP Kinematics::baseOrderConvert(MControl *mControl)
{
    ...
    converterParameters.pwm.BLDC_R = mControl->rpm.BLDC_R * (pow(2, PWM_RESOLUTION) / _iMaxRPM);
    converterParameters.pwm.BLDC_L = mControl->rpm.BLDC_L * (pow(2, PWM_RESOLUTION) / _iMaxRPM);
