...
Kinematics::Kinematics(base robot_base, int motor_max_vel,
                       ...
                       float track) : eBasePlatform(robot_base),
                                      _iMaxRPM(motor_max_vel * (60 / _fWheelCircumference) * 4),
...
Kinematics::MControl Kinematics::ackerman(MCommand *mCommand)
{
    ...
    {
        mControl.rpm.BLDC_L = (linear_x_mm / 1000) * (60 / _fWheelCircumference);
        mControl.rpm.BLDC_R = (linear_x_mm / 1000) * (60 / _fWheelCircumference);
    ...
    mControl.rpm.BLDC_L = (vel_L / 1000) * (60 / _fWheelCircumference);
    mControl.rpm.BLDC_R = (vel_R / 1000) * (60 / _fWheelCircumference);
...
Kinematics::CP Kinematics::baseOrderConvert(MControl *mControl)
{
    ...
    converterParameters.pwm.BLDC_R = (mControl->rpm.BLDC_R * BLDC_GEAR_RATIO) * (255 / _iMaxRPM);
    converterParameters.pwm.BLDC_L = (mControl->rpm.BLDC_L * BLDC_GEAR_RATIO) * (255 / _iMaxRPM);
