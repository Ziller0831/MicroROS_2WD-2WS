...
Kinematics::CP Kinematics::inverseKinematics(float linear_x, float center_rotation_rad, int turning_distance)
{
    MCommand mCommand;
    MControl mControl;
    ...
    mCommand.turning_distance = turning_distance;
...
Kinematics::MControl Kinematics::ackerman(MCommand *mCommand)
{
    ...
    if (mCommand->center_rotation_rad != 0 && mCommand->center_rotation_angle)
