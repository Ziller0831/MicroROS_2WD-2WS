...
Kinematics::Kinematics(base robot_base, int motor_max_vel,
                       ...
                       float track) : _eBasePlatform(robot_base),
                                      _iMaxRPM(motor_max_vel * (60 / fWheelCircumference_) / 1000),
                                      _fWheelBase(eBasePlatform_ == DIFFERENTIAL_DRIVE ? 0 : wheelBase),
                                      _fTrack(track),
                                      fWheelCircumference(PI * wheelDiameter * 0.0254),
                                      degree2pulse(DEG_TO_PUL){};
...
Kinematics::MControl Kinematics::selfRotation(MCommand *mCommand)
{
    ...
    mControl.angle.STEPPER_R = atan(fWheelBase_ / (fTrack_ / 2)) * RAD_TO_DEG;
    mControl.angle.STEPPER_L = -atan(fWheelBase_ / (fTrack_ / 2)) * RAD_TO_DEG;

    mControl.rpm.BLDC_L = rotation_speed;
    mControl.rpm.BLDC_R = -rotation_speed;
