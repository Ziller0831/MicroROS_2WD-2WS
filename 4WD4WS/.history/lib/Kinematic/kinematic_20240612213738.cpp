...
Kinematics::Kinematics(base robot_base, int motor_max_vel,
                       ...
                       float track) : eBasePlatform(robot_base),
                                      _iMaxRPM(motor_max_vel * (60 / _fWheelCircumference) / 1000),
...
Kinematics::CP Kinematics::baseOrderConvert(MControl *mControl)
{
    ...
    converterParameters.pulse.STEPPER_R = mControl->angle.STEPPER_R / degree2pulse;
    converterParameters.pulse.STEPPER_L = mControl->angle.STEPPER_L / degree2pulse;

    converterParameters.pwm.BLDC_R = mControl->rpm.BLDC_R * (1025 / iMaxRPM);
    converterParameters.pwm.BLDC_L = mControl->rpm.BLDC_L * (1025 / iMaxRPM);
