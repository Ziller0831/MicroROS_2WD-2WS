...
Kinematics::CP Kinematics::inverseKinematics(MCommand *mCommand)
{
    MControl mControl;
...
Kinematics::MControl Kinematics::ackerman(MCommand *mCommand)
{
    ...
    {
        angle_R_ = atan(fWheelBase_ / R_short);
        angle_L_ = atan(fWheelBase_ / R_long);

        vel_R = (R_short / cos(angle_R_)) / abs_center_rotation_rad * linear_x_mm;
        vel_L = (R_long / cos(angle_L_)) / abs_center_rotation_rad * linear_x_mm;
    }
    else if (center_rotation_rad < -800 && turning_angle > 0)
    {
        angle_R_ = atan(fWheelBase_ / R_long) * RAD_TO_DEG;
        angle_L_ = atan(fWheelBase_ / R_short) * RAD_TO_DEG;

        vel_R = (R_long / cos(angle_R_)) / abs_center_rotation_rad * linear_x_mm;
        vel_L = (R_short / cos(angle_L_)) / abs_center_rotation_rad * linear_x_mm;
    }
    else
    {
        angle_R_ = 0;
    ...
    mControl.angle.STEPPER_R = angle_R_ * RAD_TO_DEG;
    mControl.angle.STEPPER_L = angle_L_ * RAD_TO_DEG;
...
{
    angle_R_ = mControl->angle.STEPPER_R;
    angle_L_ = mControl->angle.STEPPER_L;
}