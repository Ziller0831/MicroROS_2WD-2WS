...
Kinematics::MControl Kinematics::ackerman(MCommand *mCommand)
{
    ...
    mControl.rpm.BLDC_L = constrain((vel_L / 1000),MAX_BLDC_vel, -MAX_BLDC_vel) * (60 / fWheelCircumference_);
    mControl.rpm.BLDC_R = constrain((vel_R / 1000), MAX_BLDC_vel, -MAX_BLDC_vel) * (60 / fWheelCircumference_);
...
Kinematics::CP Kinematics::baseOrderConvert(MControl *mControl)
{
    ...
    converterParameters.pwm.BLDC_R = mControl->rpm.BLDC_R * (255 / MAX_BLDC_vel);
    converterParameters.pwm.BLDC_L = mControl->rpm.BLDC_L * (255 / MAX_BLDC_vel);
