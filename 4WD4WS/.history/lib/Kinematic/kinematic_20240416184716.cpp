...
Kinematics::MControl Kinematics::ackerman(MCommand *mCommand)
{
    ...
    {
        angle_R = atan(WHEELBASE / R_short) * RAD_TO_DEG;
        angle_L = atan(WHEELBASE / R_long) * RAD_TO_DEG;
    }
    else if (center_rotation_rad < -800 && turning_angle > 0)
    {
        angle_R = atan(WHEELBASE / R_lo) * RAD_TO_DEG;
        angle_L = atan(WHEELBASE / R_long) * RAD_TO_DEG;
    }
    ...
    mControl.angle.STEPPER_R = angle_R;
    mControl.angle.STEPPER_L = angle_L;
