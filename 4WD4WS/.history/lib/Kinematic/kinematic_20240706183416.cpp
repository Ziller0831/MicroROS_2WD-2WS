...
Kinematics::MControl Kinematics::selfRotation(MCommand *mCommand)
{
    ...
    if (rotation_speed > 0)
    {
        mControl.rpm.BLDC_L = (rotation_speed / 1000) * (60 / _fWheelCircumference) * BLDC_GEAR_RATIO;
        mControl.rpm.BLDC_R = -(rotation_speed / 1000) * (60 / _fWheelCircumference) * BLDC_GEAR_RATIO;
    }
