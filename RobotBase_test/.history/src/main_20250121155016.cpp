...
void loop()
{
  ...
  if (current_time - Last_time >= Sampling_time)
  {
    ...
    las float R_speed = R_Encoder.getCount() - DEG_TO_PUL / Sampling_time;
    float L_speed = L_Encoder.getCount() * DEG_TO_PUL / Sampling_time;

    current_speed[0] = R_speed;
    current_speed[1] = L_speed;
